...
static void calc_c(double k[], double c[], int *V, double *conc){
...
static void calc_a(int Xi1, int Xi2, int Xa, int Xb, long Xatol, long Xbtol, double c[], double a[]){
...
static int chain_judge(long Xtol){
    double r=0;
    r=rand()/(RAND_MAX+1.0);
    int num;
    num=Xtol*r;
...
static void homopro(long X[], long Xtol){
    int num;
...
static int chain_delete(long X[], int num, long Xtol){
    int n;
    n=X[num];
...
static void crosspro(long Aa[], long Ab[], long Ba[], long Bb[], long Xatol, long Xbtol){
    int num, a, b;
...
static void homoter(long a[], long b[], long Pa[], long Pb[], long Xtol, int Xptol){
...
static void crosster(long Aa[], long Ab[], long Ba[], long Bb[], long Pa[], long Pb[], long Xatol, long Xbtol, int Xptol){
    int num1, num2, a1, a2, b1, b2;
...
static void trn(long A[], long B[], long Pa[], long Pb[], long Xtol, int Xptol){
    int num, a, b;
    num=chain_judge(Xtol);
...
static double calc_Mn(long *Mtemp, long Aa[], long Ab[], long Ba[], long Bb[], long Pa[], long Pb[], long Xatol, long Xbtol, int Xptol){
    ...
    for (int i=0;i<Xatol;i++){*Mtemp=*Mtemp+200*Aa[i]+200*Ab[i];}
    for (int i=0;i<Xbtol;i++){*Mtemp=*Mtemp+200*Ba[i]+200*Bb[i];}
    for (int i=0;i<Xptol;i++){*Mtemp=*Mtemp+200*Pa[i]+200*Pb[i];}
...
static double calc_MWD(long Mtemp, double Mn, long Aa[], long Ab[], long Ba[], long Bb[], long Pa[], long Pb[], long Xatol, long Xbtol, int Xptol){
    ...
    for (int i=0;i<Xatol;i++){M2=M2+(200*Aa[i]+200*Ab[i])*(200*Aa[i]+200*Ab[i]);}
    for (int i=0;i<Xbtol;i++){M2=M2+(200*Ba[i]+200*Bb[i])*(200*Ba[i]+200*Bb[i]);}
    for (int i=0;i<Xptol;i++){M2=M2+(200*Pa[i]+200*Pb[i])*(200*Pa[i]+200*Pb[i]);}
...
PyObject *Monte_therm_disp(PyObject *self, PyObject *args){
    ...
    while((Xa+Xb)>=0 && Xi1>Xi1thre){
        calc_a(Xi1, Xi2, Xa, Xb, Xatol, Xbtol,c,a);
        mu=gene_mu(a);
